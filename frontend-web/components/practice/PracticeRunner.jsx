...
function PracticeContent() {
    ...
    // Prevent accidental close/refresh - removed as per user request to avoid native notifications
    useEffect(() => {
        // We rely on Navbar's custom ConfirmationModal for internal navigation
    }, [finished]);
    ...
    const handleOptionSelect = (option) => {
        ...
        const attempt = {
            ...
            character: currentQ.character || currentQ.word || currentQ.title, // Fallback: grammar uses 'title', vocab uses 'word'
                    ...
                    <div className="mb-6">
                        ...
                        {pendingSyncs > 0 && (
                            <div className="mt-3 bg-brand/10 border border-brand/20 text-brand text-xs px-3 py-2 rounded-lg font-bold">
                                ⏳ {pendingSyncs} sesi latihan menunggu sinkronisasi kke server.
                            </div>
                        )}
                    </div>
